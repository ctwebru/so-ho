// Mixkit free CDN videos used as ambient backgrounds.
// Each drink has a video that conveys vibe, not just the product.

export type Macro = { kcal: number; protein: number; fat: number; carbs: number };
export type Size = { id: "S" | "M" | "L"; ml: number };
export type Addon = { id: string; name: string; price: number; img?: string };
export type Drink = {
  id: string;
  name: string;
  subtitle?: string;
  price: number;
  category: CategoryId;
  group: string; // sub-category label
  video: string;
  poster?: string;
  accent: string; // soft hex tint behind the video for cohesive theming
  badge?: string;
  macro: Macro;
  sizes: Size[];
  defaultSize: "S" | "M" | "L";
  milks: Addon[];
  toppings: Addon[];
  variants: Addon[]; // related variants (горячая версия, со льдом и пр.)
  flavors?: Addon[]; // выбор вкуса — карточка показывает, что внутри несколько вкусов
  soldOut?: boolean; // остатки закончились — не доступен для заказа
};

export type CategoryId = "you" | "spring" | "coffee" | "nocoffee" | "combo";
export const CATEGORIES: { id: CategoryId; label: string }[] = [
  { id: "you", label: "для тебя" },
  { id: "spring", label: "весна" },
  { id: "coffee", label: "кофе" },
  { id: "nocoffee", label: "не кофе" },
  { id: "combo", label: "комбо" },
];

// Local ambient videos used as backgrounds for cards.
import promo from "@/assets/coffee-promo.mp4";
import vPour from "@/assets/mixkit-pouring-coffee-in-a-cup-43941-hd-ready.mp4";
import vIced from "@/assets/mixkit-orange-juice-poured-over-ice-48092-hd-ready.mp4";
import vSteam from "@/assets/mixkit-cinemagraph-of-steaming-cup-of-tea-and-sweats-45086-hd-ready.mp4";
import vBeans from "@/assets/mixkit-barista-cleaning-a-coffee-machine-817-hd-ready.mp4";
import vLatteart from "@/assets/mixkit-serving-a-sparkling-cappuccino-in-a-cup-41859-hd-ready.mp4";
import vCream from "@/assets/mixkit-cup-being-filled-with-coffee-in-a-coffee-machine-41865-hd-ready.mp4";
import vMatcha from "@/assets/mixkit-mug-with-sparkling-cappuccino-5072-hd-ready.mp4";
import vHand from "@/assets/mixkit-hand-takes-cup-of-coffee-from-the-table-20029-hd-ready.mp4";
import vServed from "@/assets/mixkit-coffee-is-served-in-a-cup-3573-hd-ready.mp4";

export const PROMO_VIDEO = promo;
const V = {
  pour: vPour,
  iced: vIced,
  steam: vSteam,
  beans: vBeans,
  latteart: vLatteart,
  orange: vIced,
  matcha: vMatcha,
  cream: vCream,
  hand: vHand,
  served: vServed,
};

const sizesStd: Size[] = [
  { id: "S", ml: 250 },
  { id: "M", ml: 350 },
  { id: "L", ml: 450 },
];

const milks: Addon[] = [
  { id: "cow", name: "Молоко коровье", price: 0 },
  { id: "hazel", name: "Молоко фундучное", price: 85 },
  { id: "banana", name: "Молоко банановое", price: 85 },
  { id: "coco", name: "Молоко кокосовое", price: 85 },
  { id: "almond", name: "Молоко миндальное", price: 85 },
  { id: "oat", name: "Молоко овсяное", price: 65 },
];

const toppings: Addon[] = [
  { id: "cookie", name: "Крошка печенья", price: 10 },
  { id: "shimmer", name: "Мерцающие блёстки", price: 10 },
  { id: "gold", name: "Золотые блёстки", price: 10 },
  { id: "cinnamon", name: "Корица", price: 0 },
  { id: "cocoa", name: "Посыпка какао", price: 0 },
  { id: "raspberry", name: "Малина сублим.", price: 25 },
];

export const DRINKS: Drink[] = [
  {
    id: "cappuccino",
    name: "Капучино",
    price: 245,
    category: "coffee",
    group: "кофе с молоком",
    video: V.steam,
    accent: "#b88a5f",
    macro: { kcal: 158, protein: 7.7, fat: 8.4, carbs: 12.2 },
    sizes: sizesStd,
    defaultSize: "M",
    milks,
    toppings,
    variants: [
      { id: "hot", name: "Горячая версия", price: 0 },
      { id: "iced", name: "Со льдом", price: 0 },
      { id: "esp+", name: "Доп. эспрессо", price: 80 },
    ],
  },
  {
    id: "latte",
    name: "Латте",
    price: 245,
    category: "coffee",
    group: "кофе с молоком",
    video: V.latteart,
    accent: "#caa07a",
    macro: { kcal: 180, protein: 8.1, fat: 9.0, carbs: 14.0 },
    sizes: sizesStd,
    defaultSize: "M",
    milks,
    toppings,
    variants: [{ id: "iced", name: "Со льдом", price: 0 }],
  },
  {
    id: "raf",
    name: "Раф ванильный",
    price: 295,
    category: "coffee",
    group: "кофе с молоком",
    video: V.cream,
    accent: "#d4a87a",
    macro: { kcal: 210, protein: 5.5, fat: 12.4, carbs: 18.0 },
    sizes: sizesStd,
    defaultSize: "M",
    milks,
    toppings,
    variants: [],
  },
  {
    id: "iced-latte",
    name: "Латте со льдом",
    price: 295,
    category: "coffee",
    group: "кофе с молоком (холодный)",
    video: V.iced,
    accent: "#e0bfa0",
    badge: "хит весны",
    macro: { kcal: 165, protein: 7.0, fat: 8.0, carbs: 13.0 },
    sizes: sizesStd,
    defaultSize: "L",
    milks,
    toppings,
    variants: [],
  },
  {
    id: "bumble",
    name: "Бамбл кофе",
    subtitle: "апельсин + эспрессо",
    price: 415,
    category: "spring",
    group: "весна",
    video: V.orange,
    accent: "#e89a3a",
    badge: "новинка",
    macro: { kcal: 149, protein: 2.8, fat: 1.2, carbs: 30.8 },
    sizes: [{ id: "M", ml: 400 }],
    defaultSize: "M",
    milks: [{ id: "none", name: "Без молока", price: 0 }],
    toppings: [
      { id: "ice", name: "Со льдом", price: 0 },
      { id: "hot", name: "Горячая версия", price: 0 },
      { id: "double", name: "Холодный эспрессо", price: 80 },
    ],
    variants: [
      { id: "hot", name: "Горячая версия", price: 0 },
      { id: "ice", name: "Со льдом", price: 0 },
    ],
  },
  {
    id: "v60",
    name: "Воронка Руанда Мутетели",
    subtitle: "7 мин · фильтр",
    price: 285,
    category: "coffee",
    group: "воронка",
    video: V.pour,
    accent: "#3a2418",
    macro: { kcal: 5, protein: 0.2, fat: 0.0, carbs: 0.8 },
    sizes: [{ id: "M", ml: 300 }],
    defaultSize: "M",
    milks: [{ id: "none", name: "Без молока", price: 0 }],
    toppings: [],
    variants: [],
  },
  {
    id: "americano",
    name: "Американо",
    price: 175,
    category: "coffee",
    group: "чёрный кофе",
    video: V.pour,
    accent: "#2a1a14",
    macro: { kcal: 9, protein: 0.3, fat: 0.0, carbs: 1.6 },
    sizes: sizesStd,
    defaultSize: "M",
    milks: [{ id: "none", name: "Без молока", price: 0 }],
    toppings: [],
    variants: [{ id: "double", name: "Двойной", price: 60 }],
  },
  {
    id: "espresso",
    name: "Эспрессо",
    subtitle: "double shot",
    price: 145,
    category: "coffee",
    group: "чёрный кофе",
    video: V.beans,
    accent: "#1a0e08",
    macro: { kcal: 5, protein: 0.2, fat: 0.0, carbs: 0.8 },
    sizes: [{ id: "S", ml: 60 }],
    defaultSize: "S",
    milks: [{ id: "none", name: "Без молока", price: 0 }],
    toppings: [],
    variants: [{ id: "ristretto", name: "Ристретто", price: 0 }],
  },
  {
    id: "flat-white",
    name: "Флэт уайт",
    price: 265,
    category: "coffee",
    group: "кофе с молоком",
    video: V.latteart,
    accent: "#bd9670",
    macro: { kcal: 175, protein: 8.0, fat: 9.0, carbs: 13.0 },
    sizes: sizesStd,
    defaultSize: "M",
    milks,
    toppings,
    variants: [{ id: "iced", name: "Со льдом", price: 0 }],
  },
  {
    id: "mocha",
    name: "Мокачино",
    subtitle: "шоколад · эспрессо",
    price: 320,
    category: "coffee",
    group: "кофе с молоком",
    video: V.cream,
    accent: "#5a3a24",
    macro: { kcal: 245, protein: 7.0, fat: 11.0, carbs: 28.0 },
    sizes: sizesStd,
    defaultSize: "M",
    milks,
    toppings,
    variants: [{ id: "iced", name: "Со льдом", price: 0 }],
  },
  {
    id: "cortado",
    name: "Кортадо",
    price: 235,
    category: "coffee",
    group: "кофе с молоком",
    video: V.steam,
    accent: "#a87a52",
    macro: { kcal: 90, protein: 4.0, fat: 4.5, carbs: 6.0 },
    sizes: [{ id: "S", ml: 150 }],
    defaultSize: "S",
    milks,
    toppings,
    variants: [],
  },
  {
    id: "iced-americano",
    name: "Айс американо",
    price: 195,
    category: "coffee",
    group: "кофе с молоком (холодный)",
    video: V.iced,
    accent: "#3a2418",
    macro: { kcal: 9, protein: 0.3, fat: 0.0, carbs: 1.6 },
    sizes: sizesStd,
    defaultSize: "L",
    milks: [{ id: "none", name: "Без молока", price: 0 }],
    toppings: [],
    variants: [],
  },
  {
    id: "cold-brew",
    name: "Колд брю",
    subtitle: "12 часов настоя",
    price: 295,
    category: "coffee",
    group: "кофе с молоком (холодный)",
    video: V.iced,
    accent: "#241510",
    badge: "крафт",
    macro: { kcal: 12, protein: 0.4, fat: 0.0, carbs: 2.0 },
    sizes: [{ id: "M", ml: 350 }],
    defaultSize: "M",
    milks,
    toppings: [],
    variants: [{ id: "tonic", name: "+ тоник", price: 60 }],
  },
  {
    id: "v60-ethiopia",
    name: "Воронка Эфиопия Иргачеффе",
    subtitle: "цветочный · фильтр",
    price: 295,
    category: "coffee",
    group: "воронка",
    video: V.pour,
    accent: "#4a2818",
    badge: "новинка",
    macro: { kcal: 5, protein: 0.2, fat: 0.0, carbs: 0.8 },
    sizes: [{ id: "M", ml: 300 }],
    defaultSize: "M",
    milks: [{ id: "none", name: "Без молока", price: 0 }],
    toppings: [],
    variants: [],
  },
  {
    id: "decaf-cap",
    name: "Капучино без кофеина",
    price: 195,
    category: "nocoffee",
    group: "не кофе",
    video: V.steam,
    accent: "#a87a52",
    badge: "без кофеина",
    macro: { kcal: 140, protein: 7.0, fat: 7.5, carbs: 11.0 },
    sizes: sizesStd,
    defaultSize: "M",
    milks,
    toppings,
    variants: [],
  },
  {
    id: "matcha",
    name: "Матча латте",
    price: 350,
    category: "nocoffee",
    group: "не кофе",
    video: V.matcha,
    accent: "#7ea872",
    macro: { kcal: 130, protein: 4.0, fat: 5.0, carbs: 16.0 },
    sizes: sizesStd,
    defaultSize: "M",
    milks,
    toppings,
    variants: [],
  },
  {
    id: "cocoa",
    name: "Какао на молоке",
    subtitle: "с маршмеллоу",
    price: 245,
    category: "nocoffee",
    group: "не кофе",
    video: V.cream,
    accent: "#5a3a24",
    macro: { kcal: 230, protein: 7.0, fat: 9.0, carbs: 28.0 },
    sizes: sizesStd,
    defaultSize: "M",
    milks,
    toppings,
    variants: [],
  },
  {
    id: "chai",
    name: "Чай латте масала",
    subtitle: "пряный · согревающий",
    price: 295,
    category: "nocoffee",
    group: "не кофе",
    video: V.steam,
    accent: "#9c6a3a",
    macro: { kcal: 195, protein: 6.0, fat: 7.0, carbs: 24.0 },
    sizes: sizesStd,
    defaultSize: "M",
    milks,
    toppings,
    variants: [{ id: "iced", name: "Со льдом", price: 0 }],
  },
  {
    id: "matcha-strawberry",
    name: "Матча клубника",
    price: 395,
    category: "nocoffee",
    group: "не кофе",
    video: V.matcha,
    accent: "#c4708a",
    badge: "сезон",
    macro: { kcal: 175, protein: 4.5, fat: 5.5, carbs: 22.0 },
    sizes: sizesStd,
    defaultSize: "M",
    milks,
    toppings,
    variants: [],
  },
  {
    id: "lemonade-yuzu",
    name: "Лимонад юдзу-базилик",
    price: 285,
    category: "nocoffee",
    group: "лимонады",
    video: V.iced,
    accent: "#d4c84a",
    macro: { kcal: 95, protein: 0.2, fat: 0.0, carbs: 22.0 },
    sizes: [{ id: "M", ml: 400 }],
    defaultSize: "M",
    milks: [{ id: "none", name: "Без молока", price: 0 }],
    toppings: [],
    variants: [],
  },
  {
    id: "lemonade-berry",
    name: "Лимонад облепиха",
    price: 285,
    category: "nocoffee",
    group: "лимонады",
    video: V.orange,
    accent: "#e8923a",
    macro: { kcal: 110, protein: 0.3, fat: 0.0, carbs: 26.0 },
    sizes: [{ id: "M", ml: 400 }],
    defaultSize: "M",
    milks: [{ id: "none", name: "Без молока", price: 0 }],
    toppings: [],
    variants: [],
  },
  {
    id: "spring-cherry",
    name: "Латте сакура-вишня",
    subtitle: "ограниченная серия",
    price: 395,
    category: "spring",
    group: "весна",
    video: V.cream,
    accent: "#e89aab",
    badge: "новинка",
    macro: { kcal: 195, protein: 6.0, fat: 8.0, carbs: 24.0 },
    sizes: sizesStd,
    defaultSize: "M",
    milks,
    toppings,
    variants: [{ id: "iced", name: "Со льдом", price: 0 }],
  },
  {
    id: "spring-rose",
    name: "Раф розовый кардамон",
    price: 365,
    category: "spring",
    group: "весна",
    video: V.cream,
    accent: "#d49ab4",
    macro: { kcal: 220, protein: 5.5, fat: 13.0, carbs: 19.0 },
    sizes: sizesStd,
    defaultSize: "M",
    milks,
    toppings,
    variants: [],
  },
  {
    id: "spring-tonic",
    name: "Эспрессо тоник грейпфрут",
    price: 345,
    category: "spring",
    group: "весна",
    video: V.iced,
    accent: "#e88a6a",
    macro: { kcal: 85, protein: 0.5, fat: 0.0, carbs: 18.0 },
    sizes: [{ id: "M", ml: 350 }],
    defaultSize: "M",
    milks: [{ id: "none", name: "Без молока", price: 0 }],
    toppings: [],
    variants: [],
  },
  {
    id: "kofepass-10",
    name: "10 кофе с молоком",
    subtitle: "кофепасс на месяц",
    price: 1990,
    category: "combo",
    group: "кофепасс",
    video: V.beans,
    accent: "#1a2f3a",
    badge: "−21%",
    macro: { kcal: 0, protein: 0, fat: 0, carbs: 0 },
    sizes: [{ id: "M", ml: 0 }],
    defaultSize: "M",
    milks: [{ id: "any", name: "Любое молоко", price: 0 }],
    toppings: [],
    variants: [],
  },
  {
    id: "kofepass-black",
    name: "10 чёрный кофе",
    subtitle: "кофепасс на месяц",
    price: 1590,
    category: "combo",
    group: "кофепасс",
    video: V.pour,
    accent: "#2a1a14",
    badge: "−25%",
    macro: { kcal: 0, protein: 0, fat: 0, carbs: 0 },
    sizes: [{ id: "M", ml: 0 }],
    defaultSize: "M",
    milks: [{ id: "any", name: "Эспрессо/американо", price: 0 }],
    toppings: [],
    variants: [],
  },
  {
    id: "combo-croissant",
    name: "Капучино + круассан",
    subtitle: "утренний сет",
    price: 395,
    category: "combo",
    group: "сеты",
    video: V.steam,
    accent: "#c9a07a",
    badge: "−15%",
    macro: { kcal: 480, protein: 12.0, fat: 24.0, carbs: 52.0 },
    sizes: [{ id: "M", ml: 0 }],
    defaultSize: "M",
    milks: [{ id: "any", name: "Любое молоко", price: 0 }],
    toppings: [],
    variants: [],
  },
  {
    id: "combo-matcha-cake",
    name: "Матча + чизкейк",
    subtitle: "дневной сет",
    price: 545,
    category: "combo",
    group: "сеты",
    video: V.matcha,
    accent: "#7ea872",
    macro: { kcal: 510, protein: 9.0, fat: 22.0, carbs: 64.0 },
    sizes: [{ id: "M", ml: 0 }],
    defaultSize: "M",
    milks: [{ id: "any", name: "Любое молоко", price: 0 }],
    toppings: [],
    variants: [],
  },
  {
    id: "kofepass-filter",
    name: "10 фильтр-кофе",
    subtitle: "кофепасс для гиков",
    price: 2290,
    category: "combo",
    group: "кофепасс",
    video: V.pour,
    accent: "#3a2418",
    badge: "−20%",
    macro: { kcal: 0, protein: 0, fat: 0, carbs: 0 },
    sizes: [{ id: "M", ml: 0 }],
    defaultSize: "M",
    milks: [{ id: "any", name: "Воронка/аэропресс", price: 0 }],
    toppings: [],
    variants: [],
  },
  {
    id: "pistachio-latte",
    name: "Фисташковый латте",
    subtitle: "сезонный · ограниченная серия",
    price: 385,
    category: "spring",
    group: "весна",
    video: V.cream,
    accent: "#9cb87a",
    badge: "нет в наличии",
    soldOut: true,
    macro: { kcal: 215, protein: 7.0, fat: 11.0, carbs: 22.0 },
    sizes: sizesStd,
    defaultSize: "M",
    milks,
    toppings,
    variants: [{ id: "iced", name: "Со льдом", price: 0 }],
  },
  {
    id: "tea-cocktail-hot",
    name: "Чайный коктейль · горячий",
    subtitle: "11 вкусов на выбор",
    price: 295,
    category: "nocoffee",
    group: "чайные коктейли",
    video: V.steam,
    accent: "#a8624a",
    badge: "11 вкусов",
    macro: { kcal: 140, protein: 1.5, fat: 0.5, carbs: 32.0 },
    sizes: [
      { id: "M", ml: 350 },
      { id: "L", ml: 450 },
    ],
    defaultSize: "M",
    milks: [{ id: "none", name: "Без молока", price: 0 }],
    toppings: [],
    variants: [],
    flavors: [
      { id: "raspberry-mint", name: "Малина-мята", price: 0 },
      { id: "seabuckthorn-orange", name: "Облепиха-апельсин", price: 0 },
      { id: "cherry-cinnamon", name: "Вишня-корица", price: 0 },
      { id: "ginger-honey", name: "Имбирь-мёд", price: 0 },
      { id: "blackcurrant-thyme", name: "Чёрная смородина-чабрец", price: 0 },
      { id: "strawberry-basil", name: "Клубника-базилик", price: 0 },
      { id: "pear-cardamom", name: "Груша-кардамон", price: 0 },
      { id: "apple-cinnamon", name: "Яблоко-корица", price: 0 },
      { id: "peach-rosemary", name: "Персик-розмарин", price: 0 },
      { id: "cranberry-ginger", name: "Клюква-имбирь", price: 0 },
      { id: "fig-lavender", name: "Инжир-лаванда", price: 0 },
    ],
  },
  {
    id: "tea-cocktail-cold",
    name: "Чайный коктейль · холодный",
    subtitle: "5 вкусов на выбор",
    price: 295,
    category: "nocoffee",
    group: "чайные коктейли",
    video: V.iced,
    accent: "#4a8aa8",
    badge: "5 вкусов",
    macro: { kcal: 120, protein: 0.5, fat: 0.0, carbs: 28.0 },
    sizes: [
      { id: "M", ml: 400 },
      { id: "L", ml: 500 },
    ],
    defaultSize: "M",
    milks: [{ id: "none", name: "Без молока", price: 0 }],
    toppings: [],
    variants: [],
    flavors: [
      { id: "peach-jasmine", name: "Персик-жасмин", price: 0 },
      { id: "yuzu-mint", name: "Юдзу-мята", price: 0 },
      { id: "berry-hibiscus", name: "Ягоды-каркаде", price: 0 },
      { id: "lychee-rose", name: "Личи-роза", price: 0 },
      { id: "passion-mango", name: "Маракуйя-манго", price: 0 },
    ],
  },
  {
    id: "salted-caramel-raf",
    name: "Раф солёная карамель",
    subtitle: "скоро вернётся",
    price: 315,
    category: "coffee",
    group: "кофе с молоком",
    video: V.cream,
    accent: "#c4956a",
    badge: "скоро в наличии",
    soldOut: true,
    macro: { kcal: 230, protein: 5.5, fat: 13.0, carbs: 20.0 },
    sizes: sizesStd,
    defaultSize: "M",
    milks,
    toppings,
    variants: [],
  },
  {
    id: "banana-matcha",
    name: "Матча банан",
    subtitle: "скоро вернётся",
    price: 375,
    category: "nocoffee",
    group: "не кофе",
    video: V.matcha,
    accent: "#d4c86a",
    badge: "скоро в наличии",
    soldOut: true,
    macro: { kcal: 155, protein: 4.5, fat: 5.0, carbs: 20.0 },
    sizes: sizesStd,
    defaultSize: "M",
    milks,
    toppings,
    variants: [],
  },
];

export const FOR_YOU_IDS = ["bumble", "iced-latte", "matcha", "kofepass-10", "v60"];
